...
<head>
    ...
    <title>OdontoCenter Hap | Login</title>
    ...
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        ...
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            ...
            <form class="space-y-6" action="{{ route('login.submit') }}" method="POST">
                @csrf
                <div>
                    <label for="user" class="block text-sm/6 font-medium text-gray-900">Email address</label>
                    <div class="mt-2">
                        <input type="text" name="user" id="user" autocomplete="user" required
            ...
            <p class="mt-10 text-center text-sm/6 text-gray-500">
                no tiene cuenta ?
                <a href="{{ action([App\Http\Controllers\authController::class, 'showRegisterForm']) }}"
                    class="font-semibold text-indigo-600 hover:text-indigo-500">Registrarse</a>
            </p>
        </div>
    </div>
</body>
